  ...
  socket.ev.on('connection.update', async (update: any) => {
    ...
    if (connection === 'close') {
      ...
      if (reason === DisconnectReason.loggedOut) {
        ...
        state = { status: 'disconnected', qrBase64: null, error: null };
        console.log(`🔴 WhatsApp bağlantısı kesildi (reason=${reason}, yeniden bağlanmıyor)`);
      }
    }
  });
}
...
async function checkConsent(phone: string): Promise<void> {
  ...
  if (!parent) throw new Error('İlgili telefon numarasına ait veli kaydı bulunamadı.');
