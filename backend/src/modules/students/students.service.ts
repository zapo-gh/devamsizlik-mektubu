...
import { AppError } from '../shared/utils/../middleware/errorHandler.middleware';
    ...
    return prisma.$transaction(async (tx) => {
      ...
      if (parents && parents.length > 0) {
        for (const p of parents) {
          ...
          if (!parent) {
            parent = await tx.parent.create({ data: { userId: user.id, fullName: p.fullName.trim(), phone } });
          } else {
            parent = await tx.parent.update({ where: { id: parent.id }, data: { fullName: p.fullName.trim(), phone } });
          }
  ...
  async addParentToStudent(studentId: string, data: { fullName: string; phone: string }) {
    ...
    return prisma.$transaction(async (tx) => {
      ...
      if (!parent) {
        ...
        parent = await tx.parent.update({ where: { id: parent.id }, data: { fullName: data.fullName.trim(), phone } });
      }
